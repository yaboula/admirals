...
| v1.1 | 2026-05-09 | Backend Lead GOVT MVP server data layer implementada: nuevos `repos/govt.lua`, `services/risk_engine.lua`, `services/govt_service.lua`, `callbacks/govt.lua`; callbacks admin-gated para Census list/detail, sanctions queue/actions/freeze/fine/close, treasury page, subsidy stats/list/detail y reports data. Risk Score MVP: high single outgoing >50000, medium velocity/frozen destination, low fixed placeholder. Mutaciones sensibles usan idempotency UUID + audit ledger con actor CID, terminal virtual/IP e idempotency key. |
| v1.2 | 2026-05-09 | Integración vertical GOVT aplicada: `govtCensus`, `govtSanctions`, `govtTreasury`, `govtSubsidies`, `govtReports` pasan de `useQuery`/mocks directos a `useBankCallback`/`useBankMutation` contra callbacks reales `sonar:bank:govt:*`. `data/mock/register.ts` registra handlers GOVT equivalentes para dev fuera de FiveM. Backend `GetSanctionKpis` alineado al shape UI `{ open, critical, today, total }`. Validado `npm run typecheck`, `npm run build`, `git diff --check`. Pendientes mock-only: TaxEngine y Business/Empresas hasta callbacks reales. |

---

**FIN `FE_BACKEND_REQUESTS.md` v1.2 — ✅ TODOS LOS NODOs A.GOVT COMPLETADOS + GOVT VERTICAL INTEGRATION COMPLETE + BANK BUSINESS COCKPIT POLISHED + DB Issue #002 emitido + Backend Issue #003 CLOSED.** Govt panel real-data ready para Census, Sanctions, Movimientos/Treasury, Subsidios e Informes mediante `useBankCallback`/`useBankMutation` y callbacks `sonar:bank:govt:*`; mock registry preserva desarrollo local fuera de FiveM. TaxEngine y Empresas siguen mock-only hasta callbacks reales. Bank Business Dashboard completo en UX, con action center preview-only hasta Backend Lead implemente REQ-FE-011/015.
