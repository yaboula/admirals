...
> **Status:** 🟢 **OPEN REQUESTS / GOVT DATA LAYER MVP IN PROGRESS 2026-05-09** — DB Issue #002 emitted; Backend Issue #003 closed; first Government server callbacks implemented for REQ-FE-006/007/009/012/013/014.
> **Versionado:** v1.1 (BANK-BE GOVT MVP data layer — census, sanctions, treasury, subsidies, reports). Bump cada vez que añada/cierre items.
...
**FIN `FE_BACKEND_REQUESTS.md` v1.1 — ✅ TODOS LOS NODOs A.GOVT COMPLETADOS + BANK BUSINESS COCKPIT POLISHED + DB Issue #002 emitido + Backend Issue #003 CLOSED + GOVT MVP SERVER DATA LAYER IN PROGRESS.** Govt panel (Bureau + Census + Sanctions + TaxEngine + Empresas + Movimientos + Subsidios + Informes) — 8 nodos UI completos sobre mock layer. Bank Business Dashboard completo en UX, con action center preview-only hasta Backend Lead implemente REQ-FE-015. Core + Tier 4 backend (`audit`, `idempotency`, `accounts`, `transactions/movements`, `recipients`, `loans`, `recurring`, `portfolio`, `cards`, `auth`) ya usan `sonar_bank_*` / `sonar_*`. Primer corte backend Gobierno expone datos reales MVP para Census/Sanctions/Treasury/Subsidies/Reports; siguiente paso natural: conectar frontend govt query layer a `useBankCallback`/`useBankMutation` y atacar Empresas/REQ-FE-011/015.
