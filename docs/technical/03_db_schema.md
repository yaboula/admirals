...
> **Estado v1.2:** firmado (S1.10.x). **DRAFTs v1.3+v1.4+v1.5:** founder approved 2026-05-06 (BANK-DB.1+2+3). **Estado v2.0 LOCKED PROVISIONAL:** 🔒 DB Lead sign-off + provisional founder approval pending BANK-DB.4 deliverables review (BENCHMARK v0.5 + handoff package H1). Sign-off triple completo post Backend Lead H1 ceremony + Security Lead H2 ceremony. **Condicional clauses LOCKED PROVISIONAL:** ver §28 + `progress/BENCHMARK_BANK_DB_v1.md` §8.
