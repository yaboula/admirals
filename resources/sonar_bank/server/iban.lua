...
--    │   └──────────── 4 chars random uppercase alphanumeric
--    └──────────────── 'AD-' prefijo fijo SONAR
--
-- Checksum algorithm (SONAR canonical):
...
--                               unique vs sonar_bank_accounts.iban en DB.
...
SONAR.Bank.IBAN = SONAR.Bank.IBAN or {}
...
function IBAN.ComputeChecksum(payload)
  ...
  -- SHA-256 via MySQL (consistente con sonar_core/server/migrations.lua:82).
  local hex = SONAR.DB.Scalar('SELECT SHA2(?, 256)', { payload })
...
--   3. NO colisión con sonar_bank_accounts.iban existente.
--
...
function IBAN.Generate()
  for attempt = 1, Config.IbanGenerateMaxRetries do
    ...
    if IBAN.IsReserved(iban) then
      SONAR.Log.Debug('IBAN.Generate attempt %d hit reserved prefix, retry: %s',
        attempt, iban)
      goto continue
    end

    -- Uniqueness check vs DB.
    local existing = SONAR.DB.Scalar(
      'SELECT 1 FROM sonar_bank_accounts WHERE iban = ? LIMIT 1',
      { iban })
    if existing == nil then
      SONAR.Metrics.Counter('bank.iban.generated')
      SONAR.Metrics.Counter('bank.iban.generate_attempts', attempt)
      return iban
    end

    SONAR.Log.Warn('IBAN.Generate collision attempt %d: %s', attempt, iban)
    SONAR.Metrics.Counter('bank.iban.collisions')
    ::continue::
  end

  SONAR.Metrics.Counter('bank.iban.generate_failures')
  error(string.format(
...
-- Boot announce.
-- =============================================================================
SONAR.Log.Info('IBAN module ready (prefix=%s, charset_size=%d, max_retries=%d)',
