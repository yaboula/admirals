-- =============================================================================
-- SONAR Bank — server/init.lua
--
-- Boot orchestration. LAST script en server_scripts del fxmanifest.
--
-- Secuencia boot:
--   1. Wait SONAR.Core.WaitReady (Config.CoreWaitTimeoutMs).
--      — hard-fail si timeout (sonar_core no arrancó / DB no lista /
--        migrations no aplicadas).
--   2. Verify migration 003 aplicada (sonar_bank_accounts table existe).
...
--   4. Mark Bank._ready + emit sonar:bank:ready.
--   5. Print boot panel ASCII.
--
-- Admin commands:
--   /sonar_bank_status — muestra ready state + version + counts.
--
-- Exports cross-resource:
--   IsReady()  → boolean
--   Version()  → string
--
-- API local en VM sonar_bank (consumido por callbacks.lua):
...
-- =============================================================================
-- Boot report — ASCII panel coherente con sonar_core.
-- =============================================================================
...
local function _print_boot_report()
  ...
  local accounts_count_ok, accounts_count = pcall(function()
    return SONAR.DB.Scalar('SELECT COUNT(*) FROM sonar_bank_accounts', {})
  end)
  if not accounts_count_ok then accounts_count = '?' end

  print('')
  print(divider)
  print(string.format('^5  SONAR Bank v%s — Boot Report^7', Config.Version))
  print(divider)
  ...
  print(string.format('  Core ver   : %s', SONAR.Core.Version() or '?'))
  print(string.format('  Boot time  : %dms', boot_ms))
  print(thin)
  print(string.format('  bank_accounts rows: %s', tostring(accounts_count)))
  print(divider)
  print('^2  sonar_bank is READY^7')
  print(divider)
  print('')
end

-- =============================================================================
-- Admin command — /sonar_bank_status.
-- =============================================================================
local function _is_admin(source)
  if source == 0 then return true end
  return IsPlayerAceAllowed(source, 'command.sonar_bank_status')
end

RegisterCommand('sonar_bank_status', function(source)
  if not _is_admin(source) then return end
  print(string.format('^5[SONAR Bank] v%s | ready=%s | tracked_sources=%d^7',
...
-- =============================================================================
-- Migration sanity check — verifica que sonar_bank_accounts existe.
-- =============================================================================
local function _verify_schema_migrated()
  local exists = SONAR.DB.Scalar([[
    SELECT COUNT(*) FROM information_schema.tables
    WHERE table_schema = DATABASE() AND table_name = 'sonar_bank_accounts'
  ]], {})
  if (tonumber(exists) or 0) == 0 then
    error('[sonar_bank] sonar_bank_accounts table NOT FOUND — migration 003 missing or failed', 0)
  end

  local exists_mov = SONAR.DB.Scalar([[
    SELECT COUNT(*) FROM information_schema.tables
    WHERE table_schema = DATABASE() AND table_name = 'sonar_bank_movements'
  ]], {})
  if (tonumber(exists_mov) or 0) == 0 then
    error('[sonar_bank] sonar_bank_movements table NOT FOUND — migration 003 missing or failed', 0)
  end
end
...
local function _register_identity_hooks()
  SONAR.Identity.OnPlayerLoaded(function(citizen_id, source)
    if type(citizen_id) ~= 'string' or citizen_id == '' then return end
    local src_num = tonumber(source) or -1
    _src_to_cid[src_num] = citizen_id

    SONAR.Log.Debug('Identity loaded: source=%d citizen_id=%s', src_num, citizen_id)
    SONAR.Metrics.Counter('bank.identity.loaded')
    ...
    Citizen.CreateThread(function()
      ...
      if not ok then
        SONAR.Log.Error('EnsureStarterAccount failed for %s: %s', citizen_id, tostring(result))
      else
        if result.created then
          SONAR.Log.Info('Starter account created for %s: %s (balance=%s €)',
            citizen_id, result.iban, result.balance)
        else
          SONAR.Log.Debug('Starter account already exists for %s: %s', citizen_id, result.iban)
        end
      end
    end)
  end)

  SONAR.Identity.OnPlayerDropped(function(citizen_id, source, reason)
    local src_num = tonumber(source) or -1
    _src_to_cid[src_num] = nil
    SONAR.Log.Debug('Identity dropped: source=%d citizen_id=%s reason=%s',
      src_num, tostring(citizen_id), tostring(reason))
    SONAR.Metrics.Counter('bank.identity.dropped')
  end)

  SONAR.Log.Info('Identity hooks registered (OnPlayerLoaded + OnPlayerDropped)')
end
...
CreateThread(function()
  ...
  SONAR.Log.Info('sonar_bank v%s booting...', Config.Version)

  -- ---------------------------------------------------------------------------
  -- 1. Wait sonar_core ready.
  -- ---------------------------------------------------------------------------
  SONAR.Log.Debug('Waiting sonar_core...')
  local core_ready = SONAR.Core.WaitReady(Config.CoreWaitTimeoutMs)
  if not core_ready then
    SONAR.Log.Error('sonar_core not ready after %dms — aborting boot',
      Config.CoreWaitTimeoutMs)
    error('[sonar_bank] sonar_core not ready', 0)
  end
  SONAR.Log.Info('sonar_core ready (v%s)', SONAR.Core.Version() or '?')
  ...
  if not schema_ok then
    SONAR.Log.Error('Schema sanity check failed: %s', tostring(schema_err))
    error(tostring(schema_err), 0)
  end
  SONAR.Log.Info('Schema verified: sonar_bank_accounts + sonar_bank_movements OK')
  ...
  SONAR.Metrics.Counter('bank.boot_success')
  SONAR.Metrics.Gauge('bank.boot_duration_ms', _boot_completed_at - _boot_started_at)

  SONAR.Bus.Publish(Config.BankReadyEventName, {
