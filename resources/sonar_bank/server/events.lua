-- =============================================================================
-- SONAR Bank — server/events.lua
--
-- Helpers de publicación de eventos canónicos via SONAR.Bus.Publish.
...
--   3. SONAR.Bus.Publish — auto-decora `_event_name, _event_id, _emitted_at,
--      _schema_version` per `02_events_catalog.md` §1.4.
--   4. Wrap sonar_core en su Bus.Publish ya hace fanout server-wide via
--      TriggerEvent('sonar_lib:dispatch') — los consumers en otros resources
...
--   S2+ podrá usar SONAR.Bus.RegisterSchema con validators tipados completos
...
SONAR.Bank.Events = SONAR.Bank.Events or {}

local Config = SONAR.Bank.Config
local Events = SONAR.Bank.Events
...
--
-- movement_id_from/movement_id_to NO se emiten en S1.2 — SONAR.DB.Transaction
...
-- (ej. sonar_documents auto-receipt generation).
-- =============================================================================
function Events.PublishTransferCompleted(payload)
  ...
  if not ok then
    SONAR.Log.Error('PublishTransferCompleted: missing field "%s"', tostring(missing))
    SONAR.Metrics.Counter('bank.events.transfer_completed.invalid_payload')
    return false
  end
  ...
  SONAR.Bus.Publish('sonar:bank:transfer_completed', final_payload, {
    audit = 'always',  -- per SSoT §4.3 "Audit: always"
  })

  SONAR.Metrics.Counter('bank.events.transfer_completed.published')
  return true
end
...
function Events.PublishEscrowCreated(payload)
  ...
  if not ok then
    SONAR.Log.Error('PublishEscrowCreated: missing field "%s"', tostring(missing))
    SONAR.Metrics.Counter('bank.events.escrow_created.invalid_payload')
    return false
  end

  SONAR.Bus.Publish(Config.Events.EscrowCreated, {
  ...
  SONAR.Metrics.Counter('bank.events.escrow_created.published')
  return true
end
...
function Events.PublishEscrowReleased(payload)
  ...
  if not ok then
    SONAR.Log.Error('PublishEscrowReleased: missing field "%s"', tostring(missing))
    SONAR.Metrics.Counter('bank.events.escrow_released.invalid_payload')
    return false
  end

  SONAR.Bus.Publish(Config.Events.EscrowReleased, {
    escrow_id              = payload.escrow_id,
    buyer_iban             = payload.buyer_iban,
    seller_iban            = payload.seller_iban,
    amount                 = payload.amount,
  ...
  SONAR.Metrics.Counter('bank.events.escrow_released.published')
  return true
end
...
function Events.PublishEscrowRefunded(payload)
  ...
  if not ok then
    SONAR.Log.Error('PublishEscrowRefunded: missing field "%s"', tostring(missing))
    SONAR.Metrics.Counter('bank.events.escrow_refunded.invalid_payload')
    return false
  end

  SONAR.Bus.Publish(Config.Events.EscrowRefunded, {
    escrow_id              = payload.escrow_id,
    buyer_iban             = payload.buyer_iban,
    seller_iban            = payload.seller_iban,
    amount                 = payload.amount,
  ...
  SONAR.Metrics.Counter('bank.events.escrow_refunded.published')
  return true
end
...
-- Boot announce.
-- =============================================================================
SONAR.Log.Info('Events module ready (transfer_completed §4.3 + escrow_created/released/refunded S1.3)')
