...
--   docs/technical/03_db_schema.md §4.1 (sonar_bank_accounts).
...
SONAR.Bank.Config = SONAR.Bank.Config or {}

local Config = SONAR.Bank.Config

-- -----------------------------------------------------------------------------
-- Version — SEMVER de sonar_bank. Bump MINOR si nueva callback, MAJOR si
...
Config.StarterMovementConcept = 'Saldo inicial SONAR'

-- Categoría del movimiento — usa enum valor canónico sonar_bank_movements.category.
...
-- sonar_bank_movements.request_nonce CHAR(36). El prefijo 'starter_' fue
...
-- Categorías canónicas escritas a sonar_audit_log via SONAR.Log.Audit.
...
-- Si true, cada C001 getBalance escribe a sonar_audit_log. Default false
...
-- Timeout esperando sonar_core ready antes de registrar callbacks/identity hook.
Config.CoreWaitTimeoutMs = 30000

-- Evento ready que sonar_bank emite cuando ya está operativo.
Config.BankReadyEventName = 'sonar:bank:ready'

-- Eventos canónicos que sonar_bank emite (publica al Bus sonar_core):
Config.Events = {
  AccountCreated         = 'sonar:bank:account_created',
  StarterBalanceCredited = 'sonar:bank:starter_balance_credited',
  -- S1.2:
  TransferCompleted      = 'sonar:bank:transfer_completed',
  -- S1.3:
  EscrowCreated          = 'sonar:bank:escrow_created',
  EscrowReleased         = 'sonar:bank:escrow_released',
  EscrowRefunded         = 'sonar:bank:escrow_refunded',
}
