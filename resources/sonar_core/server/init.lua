...
--      — hard-fail si timeout (sonar_bridges no arrancó correctamente).
--   2. Wait oxmysql ready via SONAR.DB.WaitReady
--      — hard-fail si timeout (DB no conecta).
--   3. SONAR.Migrations.RunAll()
--      — hard-fail si error (schema corrupto).
--   4. Mark SONAR.Core._ready = true.
--   5. Emit sonar:core:ready (TriggerEvent + SONAR.Bus.Publish).
--   6. Print boot report (versiones, migrations aplicadas, log level).
--
-- Admin commands:
--   /sonar_core_status — muestra ready state + version + migrations list.
--
-- Exports:
--   IsReady()            → boolean
...
local DB = SONAR.DB
local Bus = SONAR.Bus
local Migrations = SONAR.Migrations
local Core = SONAR.Core
...
--   exports.sonar_core:IsReady()
--   exports.sonar_core:WaitReady(10000)
...
-- =============================================================================
-- Cross-resource exports — para consumers vía lib/sonar.lua (S1.1+).
--
-- Los consumers no pueden leer _G.SONAR (Lua VMs aisladas). Estos exports
...
-- ----------------------------------------------------------------------------
-- DB layer (6 exports — pass-through a SONAR.DB).
-- ----------------------------------------------------------------------------
...
-- Bus layer (4 exports).
--
-- BusPublish: dispatch a subscribers locales en sonar_core's VM + fanout
--   server-wide via TriggerEvent('sonar_lib:dispatch', event, payload) para
--   que consumers que cargan lib/sonar.lua reciban en sus AddEventHandler.
...
-- En su lugar, lib/sonar.lua mantiene _local_subs en VM consumer + escucha
...
-- ----------------------------------------------------------------------------
exports('RateCheck',          function(identity, bucket_key) return SONAR.Rate.Check(identity, bucket_key) end)
exports('RateRegisterBucket', function(key, def)              return SONAR.Rate.RegisterBucket(key, def) end)
exports('RateReset',          function(identity, bucket_key) return SONAR.Rate.Reset(identity, bucket_key) end)
exports('RateStats',          function() return SONAR.Rate.Stats() end)
...
--
-- Si el caller usa el _G.Log directo dentro de sonar_core, mantiene la
...
-- sonar_bridges/server/dispatcher.lua (`Bridges.SetIdempotencyBackend`).
--
-- Persisten en `sonar_bridge_idempotency` table (creada en migration 002,
...
-- Cleanup vía export GC (sonar_bridges GC thread cada 5 min).
-- ----------------------------------------------------------------------------
...
exports('IdempotencySet', function(key, result_table, ttl_sec)
  ...
  DB.Execute([[
    INSERT INTO sonar_bridge_idempotency
...
exports('IdempotencyGC', function()
  ...
  local affected = DB.Execute(
    'DELETE FROM sonar_bridge_idempotency WHERE expires_at < UNIX_TIMESTAMP()',
    {}
  )
...
--   2. Si OK, dispara TriggerEvent('sonar_lib:dispatch', event_name, payload)
--      → todos los resources con lib/sonar.lua cargada reciben el dispatch
...
Bus.Publish = function(event_name, payload, opts)
  ...
  if ok then
    ...
    -- consumer son contained en sus pcalls (lib/sonar.lua _local_subs handler).
    TriggerEvent('sonar_lib:dispatch', event_name, payload)
  end
  return ok
end

-- -----------------------------------------------------------------------------
-- Boot report — ASCII panel de cierre, coherente con sonar_bridges §10.4.
-- -----------------------------------------------------------------------------
local function _print_boot_report()
  ...
  print('')
  print(divider)
  print(string.format('^5  SONAR Core v%s — Boot Report^7', Config.Version))
  print(divider)
  ...
  print(divider)
  print('^2  sonar_core is READY^7')
  print(divider)
  print('')
end

-- -----------------------------------------------------------------------------
-- Admin command — /sonar_core_status.
-- -----------------------------------------------------------------------------
...
RegisterCommand('sonar_core_status', function(source)
  if not _is_admin(source) then return end
  print(string.format('^5[SONAR Core] v%s | ready=%s | boot_ms=%d^7',
  ...
  local rate_stats = SONAR.Rate.Stats()
...
CreateThread(function()
  ...
  Log.Info('sonar_core v%s booting...', Config.Version)

  -- -------------------------------------------------------------------------
  -- 1. Wait Bridges ready.
  -- -------------------------------------------------------------------------
  Log.Debug('Waiting for sonar_bridges...')
  local bridges_exports_ok, bridges = pcall(function()
    return exports.sonar_bridges
  end)
  if not bridges_exports_ok then
    Log.Error('sonar_bridges exports not available — dependency broken')
    error('[sonar_core] sonar_bridges not started', 0)
  end

  -- Cross-resource: each FiveM resource has its own Lua VM, so _G.Bridges
  -- is NOT shared. Consumers MUST go through exports.sonar_bridges.
  local br_ready = false
  local wait_ok, wait_err = pcall(function()
    br_ready = exports.sonar_bridges:WaitReady(Config.BridgesWaitTimeoutMs) == true
  end)
  if not wait_ok then
    Log.Error('exports.sonar_bridges:WaitReady call failed: %s', tostring(wait_err))
    error('[sonar_core] Bridges export call failed', 0)
  end

  if not br_ready then
    Log.Error('sonar_bridges not ready after %dms — aborting boot',
      Config.BridgesWaitTimeoutMs)
    error('[sonar_core] Bridges not ready', 0)
  end

  -- Snapshot active adapters for boot report.
  local active_ok, active_snap = pcall(function()
    return exports.sonar_bridges:GetActive()
  end)
  if active_ok and type(active_snap) == 'table' then
    local parts = {}
    for k, v in pairs(active_snap) do parts[#parts + 1] = k .. '=' .. tostring(v) end
    Log.Info('sonar_bridges ready. Active: %s', table.concat(parts, ', '))
  else
    Log.Info('sonar_bridges ready')
  end
  ...
  if not db_ok then
    Log.Error('DB not reachable after %dms: %s', Config.BridgesWaitTimeoutMs,
      tostring(db_reason))
    error('[sonar_core] DB not ready: ' .. tostring(db_reason), 0)
  end
  Log.Info('DB ready (oxmysql ping OK)')

  -- -------------------------------------------------------------------------
  -- 3. Pre-step: namespace upgrade (Phase 9 / ADR-013).
  --    Si sonar_schema_versions existe y sonar_schema_versions NO existe
  --    todavía, renombrar antes de que el runner intente leer sonar_.
  --    Idempotente: RENAME solo corre si origen existe + destino no existe.
  --    Fresh installs: sonar_ nunca existió → no-op seguro.
  -- -------------------------------------------------------------------------
  local _legacy_exists = DB.Scalar([[
    SELECT COUNT(*) FROM information_schema.tables
    WHERE table_schema = DATABASE() AND table_name = 'sonar_schema_versions'
  ]], {})
  local _sonar_exists = DB.Scalar([[
    SELECT COUNT(*) FROM information_schema.tables
    WHERE table_schema = DATABASE() AND table_name = 'sonar_schema_versions'
  ]], {})
  if (tonumber(_legacy_exists) or 0) > 0 and (tonumber(_sonar_exists) or 0) == 0 then
    Log.Info('Pre-step: renaming sonar_schema_versions → sonar_schema_versions (ADR-013 upgrade)')
    local ok_rename = pcall(function()
      MySQL.query.await('RENAME TABLE sonar_schema_versions TO sonar_schema_versions', {})
    end)
    if not ok_rename then
      Log.Error('Pre-step: RENAME sonar_schema_versions failed — aborting boot')
      error('[sonar_core] sonar_schema_versions rename failed', 0)
    end
    Log.Info('Pre-step: rename OK')
  end
  ...
  if not ok_mig then
    ...
    error('[sonar_core] Migrations failed', 0)
  end
  ...
  if #_migration_report.errors > 0 then
    ...
    if Config.MigrationsFailFast then
      error('[sonar_core] Migrations reported errors', 0)
    end
  end

  -- -------------------------------------------------------------------------
  -- 3.5. Swap sonar_bridges idempotency backend a DB-backed (S1.2).
  --
  -- Pre-condition: DB ready + migration 002 aplicada (sonar_bridge_idempotency
  ...
  local idem_swap_ok, idem_swap_err = pcall(function()
    return exports.sonar_bridges:SetIdempotencyBackend({
      resource  = 'sonar_core',
  ...
  elseif idem_swap_err == false then
    ...
    Log.Info('Bridges idempotency backend swapped to DB-backed (sonar_bridge_idempotency)')
  end
  ...
  --    b) SONAR.Bus.Publish para sub interno SONAR.
  -- -------------------------------------------------------------------------
