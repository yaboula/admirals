-- =============================================================================
-- SONAR Core — server/db.lua
--
-- Wrapper canónico oxmysql. TODA query SONAR debe pasar por aquí.
--
-- API pública (todas síncronas — await internamente vía oxmysql *.await):
--   SONAR.DB.FetchOne(query, params)      → row | nil
--   SONAR.DB.FetchAll(query, params)      → array
--   SONAR.DB.Execute(query, params)       → affectedRows
--   SONAR.DB.Insert(query, params)        → insertId (LAST_INSERT_ID)
--   SONAR.DB.Scalar(query, params)        → primera columna de primera row
--   SONAR.DB.Transaction(queries[])       → boolean success
--
...
local function _validate(query, params, caller)
  if type(query) ~= 'string' or query == '' then
    error(('[SONAR.DB.%s] query must be non-empty string'):format(caller or '?'), 3)
  end
  if type(params) ~= 'table' then
    error(('[SONAR.DB.%s] params must be a table (use {} if none)'):format(caller or '?'), 3)
  end
  ...
  if param_count > 0 then
    if not query:find('?', 1, true) then
      error(('[SONAR.DB.%s] params provided but query has no "?" placeholder — potential SQL injection'):format(caller or '?'), 3)
    end
  end
end
...
function DB.Transaction(queries)
  if type(queries) ~= 'table' or #queries == 0 then
    error('[SONAR.DB.Transaction] queries must be non-empty array', 2)
  end

  for i, q in ipairs(queries) do
    if type(q) ~= 'table' or type(q.query) ~= 'string' then
      error(('[SONAR.DB.Transaction] queries[%d] must be { query=string, values=table }'):format(i), 2)
    end
