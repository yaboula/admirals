-- =============================================================================
-- SONAR Core — server/event_bus.lua
--
-- Pub/sub interno SONAR. Implementa el patrón canonical definido en
-- docs/technical/01_architecture.md §5.5.
--
-- API pública:
--   SONAR.Bus.Subscribe(event_name, handler, opts?)  → sub_id (string)
--   SONAR.Bus.Unsubscribe(sub_id)                    → boolean
--   SONAR.Bus.Publish(event_name, payload, opts?)    → boolean
--   SONAR.Bus.RegisterSchema(event_name, validator)  — opcional.
--   SONAR.Bus.Stats()                                → { events, subs, ... }
--
...
-- Canonical: 'sonar:domain:action' per §01 §5.4.
-- -----------------------------------------------------------------------------
...
function Bus.Subscribe(event_name, handler, opts)
  ...
  if not ok then
    error('[SONAR.Bus.Subscribe] ' .. err, 2)
  end
...
function Bus.RegisterSchema(event_name, validator)
  if type(validator) ~= 'function' then
    error('[SONAR.Bus.RegisterSchema] validator must be function', 2)
  end
