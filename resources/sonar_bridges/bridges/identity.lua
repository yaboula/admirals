-- =============================================================================
-- SONAR Bridges — bridges/identity.lua
--
-- Bridges.Identity — resolver identidad del player:
--   source (server id efímero) ↔ citizenId (stable SSoT SONAR) ↔ data.
--
-- Responsabilidad (per doc §7.1):
--   SONAR usa `citizenId` como SSoT en toda su DB. Este bridge es el ÚNICO
...
--   SONAR se consultan via sonar_core APIs (S1+).
--
...
--     'sonar:bridge:_identityPlayerLoaded'(citizenId, source)
--     'sonar:bridge:_identityPlayerDropped'(citizenId, source, reason)
...
--- Bridges.Identity.GetJob — job framework (NO empresa SONAR).
---@param citizenId string
