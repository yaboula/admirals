-- =============================================================================
-- SONAR Bridges — adapters/phone/native.lua
--
-- Adapter NATIVE (fallback) para Phone.
--
-- Comportamiento (per doc §6.3 + §11.2):
--   SendNotification → log + StateBag `sonar:phone:lastNotification:<citizenId>`
--                      (cuando Tablet S1+ lo consumirá).
--   SendSMS          → log + StateBag `sonar:phone:lastSMS:<citizenId>`.
...
function NativePhone.SendNotification(citizenId, opts)
  ...
  opts.title = opts.title or '[SONAR]'
  ...
  GlobalState[('sonar:phone:lastNotification:%s'):format(citizenId)] = {
    title = opts.title,
...
function NativePhone.SendSMS(citizenId_to, citizenId_from, message)
  ...
  GlobalState[('sonar:phone:lastSMS:%s'):format(citizenId_to)] = {
    from = citizenId_from,
    message = message,
    ts = os.time(),
  }
