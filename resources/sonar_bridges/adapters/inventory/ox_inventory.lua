-- =============================================================================
-- SONAR Bridges — adapters/inventory/ox_inventory.lua
--
...
--   Campos sonar_item_id, quality, quality_score, lineage_* se almacenan
...
---@param count number
---@param metadata table|nil { sonar_item_id, quality, lineage_origin, ... }
---@return boolean success
...
-- -----------------------------------------------------------------------------
-- RemoveItem
-- Si sonar_item_id especificado, filtra por metadata para remover el item
-- específico (permite remover instancia exacta con lineage preservado).
-- -----------------------------------------------------------------------------
---@param citizenId string
---@param item_name string
---@param count number
---@param sonar_item_id string|nil
---@return boolean success
---@return string|nil error 'PLAYER_OFFLINE'|'INSUFFICIENT_QUANTITY'|'FAILED'
function OxInventory.RemoveItem(citizenId, item_name, count, sonar_item_id)
  if type(citizenId) ~= 'string' or citizenId == '' then
    return false, 'VALIDATION_FAILED'
  end
  local source = _get_source(citizenId)
  if not source then return false, 'PLAYER_OFFLINE' end
  count = count or 1
  local metadata = sonar_item_id and { sonar_item_id = sonar_item_id } or nil
...
-- -----------------------------------------------------------------------------
---@param citizenId string
---@param filter table|nil { item_name?, sonar_item_id? }
...
function OxInventory.GetItems(citizenId, filter)
  ...
  for _, item in ipairs(raw) do
    ...
    if include and filter.sonar_item_id then
      local mid = item.metadata and item.metadata.sonar_item_id
