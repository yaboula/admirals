...
function Bridges.PrintBootReport()
  ...
  print('')
  print(divider)
  print(string.format('^5  SONAR Bridges v%s — Configuration Report^7', Config.Version))
  print(divider)
...
CreateThread(function()
  ...
  Logger.Info('SONAR Bridges v%s booting...', Config.Version)
  ...
  -- 7) Mark ready + fire global event para sonar_core (S0.4+).
  Bridges._ready = true
  TriggerEvent('sonar:bridge:ready', detected)
...
-- -----------------------------------------------------------------------------
-- Bridges.IsReady — para consumers (sonar_core) check antes de usar.
-- -----------------------------------------------------------------------------
...
-- consumers (sonar_core, future sonar_*) cannot read _G.Bridges.
-- They must call:
