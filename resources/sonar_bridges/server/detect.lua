-- =============================================================================
-- SONAR Bridges — server/detect.lua
--
...
--   2. ApplyOverrides()— convars `sonar_bridge_<module>` + CustomAdapters.
  ...
  for _, module in ipairs(Config.Modules) do
    ...
    if override ~= '' then
      if Bridges.GetAdapter(module, override) then
        ...
        Logger.Warn('Override[convar] sonar_bridge_%s=%s: adapter not registered, keeping "%s"',
  ...
  for _, module in ipairs(Config.Modules) do
    ...
    if #started > 1 then
      Logger.Warn('Conflict[%s]: multiple resources started [%s], first-priority wins. '
        .. 'Use convar sonar_bridge_%s to override.',
