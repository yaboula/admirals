-- =============================================================================
-- SONAR Bridges — server/dispatcher.lua
--
...
-- single-server transient idem). sonar_core en su boot post-DB-ready llama
-- `Bridges.SetIdempotencyBackend({ resource='sonar_core', getExport='IdempotencyGet',
-- setExport='IdempotencySet', gcExport='IdempotencyGC' })` que swappea a DB-backed
-- (`sonar_bridge_idempotency` table — survives reboots).
--
-- Cross-VM constraint:
--   sonar_bridges no depende de sonar_core (jerarquía bridges → core).
--   Las VMs Lua FiveM son aisladas — `_G.SONAR` no es visible aquí.
...
--
-- Llamado por sonar_core en boot post-DB-ready para promover a DB-backed.
...
-- directamente vía SQL en sonar_core.)
-- -----------------------------------------------------------------------------
...
-- =============================================================================
-- Cross-resource exports — Lua VMs aisladas; consumers (sonar_bank etc.)
...
-- IsIdemReplay export — firmando para uso desde sonar_bank/server/callbacks.lua
-- C002 transfer flow.
--
