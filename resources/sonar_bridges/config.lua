...
-- -----------------------------------------------------------------------------
-- Log level — convar `sonar_bridge_log_level` (debug|info|warn|error).
-- Default: info en dev, warn en production (controlado por `sonar_env`).
-- -----------------------------------------------------------------------------
local _env = GetConvar('sonar_env', 'development')
local _default_log = _env == 'production' and 'warn' or 'info'
Config.LogLevel = GetConvar('sonar_bridge_log_level', _default_log)

-- -----------------------------------------------------------------------------
-- Boundary logging — audit log de CADA bridge call (adapter, method, latency).
--
-- Global toggle:         `sonar_bridge_log_boundary` (0|1)
-- Per-module override:   `sonar_bridge_log_boundary_<module>` (0|1)
--
-- En production, global=0 (overhead negligible pero ruidoso). En dev/debug
-- activar global=1 o per-module según necesidad.
-- -----------------------------------------------------------------------------
Config.LogBoundaryGlobal = GetConvarInt('sonar_bridge_log_boundary', 0) == 1
Config.LogBoundaryPerModule = {}
for _, module in ipairs(Config.Modules) do
  Config.LogBoundaryPerModule[module] =
    GetConvarInt('sonar_bridge_log_boundary_' .. module, 0) == 1
end
...
-- promovido a DB-backed en S1.2 junto con sonar_bridge_idempotency table
...
--   'db'     — sonar_bridge_idempotency table (DB-backed, persiste cross-restart).
--
...
-- sonar_core lo invoca en su boot post-DB-ready apuntando a 'db'.
--
-- Razón cross-resource:
--   `sonar_bridges` no puede usar `SONAR.DB` (no depende de sonar_core,
--   y _G.SONAR no está en su VM). El swap llega vía hook con resource +
--   export names (strings — NO function refs cross-VM, fragility-prone).
-- -----------------------------------------------------------------------------
Config.IdempotencyBackend = GetConvar('sonar_bridge_idempotency_backend', 'memory')
