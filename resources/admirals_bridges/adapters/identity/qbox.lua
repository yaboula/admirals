-- =============================================================================
-- Admirals Bridges — adapters/identity/qbox.lua
--
...
-- -----------------------------------------------------------------------------
-- GetJob — job framework SOLO (NO empresa Admirals per doc §7.3).
-- -----------------------------------------------------------------------------
---@param citizenId string
...
-- =============================================================================
-- Lifecycle hooks — QBox events → eventos internos admirals:bridge:_identity*.
...
AddEventHandler('QBCore:Server:PlayerLoaded', function(Player)
  ...
  if src and cid then
    _cache(src, cid)
    TriggerEvent('admirals:bridge:_identityPlayerLoaded', cid, src)
...
AddEventHandler('QBCore:Server:PlayerLogout', function(src)
  ...
  local cid = _src_to_cid[src]
  if cid then
    TriggerEvent('admirals:bridge:_identityPlayerDropped', cid, src, 'logout')
...
AddEventHandler('playerDropped', function(reason)
  ...
  local cid = _src_to_cid[src]
  if cid then
    TriggerEvent('admirals:bridge:_identityPlayerDropped', cid, src, reason or 'dropped')
