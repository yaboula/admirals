-- =============================================================================
-- Admirals Bridges — adapters/bank/qbox.lua
--
...
--   Caller (admirals_core S0.4+) genera idempotency_key único por operación.
--
-- Referencias SSoT:
...
function QboxBank.AddMoney(identifier, amount, reason, idempotency_key)
  return _with_idem(idempotency_key, function()
    ...
    local ok = Player.Functions.AddMoney('bank', amount, reason or 'admirals')
...
function QboxBank.RemoveMoney(identifier, amount, reason, idempotency_key)
  return _with_idem(idempotency_key, function()
    ...
    local ok = Player.Functions.RemoveMoney('bank', amount, reason or 'admirals')
    return ok, ok and nil or 'FAILED'
  end)
end
...
function QboxBank.Transfer(from, to, amount, reason, idempotency_key)
  return _with_idem(idempotency_key, function()
    ...
    if balance < amount then return false, 'INSUFFICIENT_FUNDS' end
    local ok_remove = PlayerFrom.Functions.RemoveMoney('bank', amount, reason or 'admirals_transfer')
    if not ok_remove then return false, 'FAILED' end
    local ok_add = PlayerTo.Functions.AddMoney('bank', amount, reason or 'admirals_transfer')
    if not ok_add then
      PlayerFrom.Functions.AddMoney('bank', amount, 'admirals_transfer_rollback')
