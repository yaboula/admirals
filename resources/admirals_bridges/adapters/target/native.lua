-- =============================================================================
-- Admirals Bridges — adapters/target/native.lua
--
...
--   code scope en S0.4 con admirals_core.
--
-- Comportamiento planeado S0.4+:
--   Distance-check + keypress E + marker ground via client script mínimo
--   consumiendo estas zones registradas (admirals_core/client/target_native.lua).
--
-- Referencias SSoT:
...
local function _warn_once()
  ...
  Logger.Warn('NativeTarget is a stub (server-only). Client-side distance-check '
    .. 'comes with admirals_core S0.4+. Install ox_target for full UX.')
end
