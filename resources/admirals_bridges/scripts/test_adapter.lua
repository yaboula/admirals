-- =============================================================================
-- Admirals Bridges — scripts/test_adapter.lua
--
-- Test harness skeleton per doc §12.5.
--
-- USO (en consola FiveM server):
--   exec resources/admirals_bridges/scripts/test_adapter.lua
--   admirals_test_adapter bank qbox
--   admirals_test_adapter inventory ox_inventory
--   admirals_test_adapter phone lb_phone
--   admirals_test_adapter identity qbox
--   admirals_test_adapter target ox_target
--   admirals_test_adapter notify ox_lib
--
-- ESTADO S0.3:
...
--   admirals_core provee ciudadanos de test y mocks de DB.
--
...
local TEST_CITIZEN  = 'TEST_CID_ADMIRALS'  -- citizenId de jugador de prueba (setup manual)
...
local function _header(module, adapter_name)
  print('^6═══════════════════════════════════════════════^7')
  print(string.format('^6  Admirals Test Harness — %s / %s^7', module, adapter_name))
  print('^6═══════════════════════════════════════════════^7')
  _passed = 0
  _failed = 0
end
...
local function _tests_inventory(adapter)
  _run('RegisterItem — valid spec returns true', function()
    -- TODO S0.4: local ok = adapter.RegisterItem({name='admirals_test_wheat', label='Test Wheat', weight=100})
  ...
  _run('GiveItem — PLAYER_OFFLINE for unknown citizenId', function()
    -- TODO S0.4: local ok, err = adapter.GiveItem('OFFLINE_CID', 'admirals_test_wheat', 1, nil)
  ...
  _run('HasItem — returns false 0 for item not in inventory', function()
    -- TODO S0.4: local has, count = adapter.HasItem(TEST_CITIZEN, 'admirals_test_wheat', 1)
...
-- =============================================================================
-- Console command: admirals_test_adapter <module> <adapter_name>
-- =============================================================================

RegisterCommand('admirals_test_adapter', function(src, args)
  if src ~= 0 then return end  -- solo consola server
  local module       = args[1]
  local adapter_name = args[2]
  if not module or not adapter_name then
    print('^3[USAGE]^7 admirals_test_adapter <module> <adapter_name>')
    print('  Modules:  bank | inventory | phone | identity | target | notify')
    print('  Example:  admirals_test_adapter bank qbox')
    return
  end
  RunTests(module, adapter_name)
end, true)

print('^6[admirals_bridges]^7 test_adapter harness loaded — use: admirals_test_adapter <module> <name>')
