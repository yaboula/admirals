...
--   locked    → disputed     — requires admirals_contracts + dispute callbacks (S2+).
...
Admirals.Bank.FSMEscrow = Admirals.Bank.FSMEscrow or {}
...
local _TRANSITIONS = {
  ...
  -- S1.3 NO implementa locked→disputed (requires admirals_contracts + C006
...
function FSM.LogTransition(escrow_id, from, to, context)
  Admirals.Log.Debug('FSMEscrow transition: escrow=%s %s→%s context=%s',
    escrow_id, from, to, tostring(context and json and json.encode(context) or ''))
  Admirals.Metrics.Counter(('bank.escrow.transition.%s_to_%s'):format(from, to))
end

-- =============================================================================
-- Boot announce.
-- =============================================================================
Admirals.Log.Info('FSMEscrow module ready (states=5, transitions S1.3=3: created→locked, locked→released|refunded)')
