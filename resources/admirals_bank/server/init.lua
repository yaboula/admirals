...
-- ==========================================
-- S1.1 SMOKE TEST TEMPORAL COMMAND
-- ==========================================
RegisterCommand('admirals_smoke_iban_gen', function(source)
    if source ~= 0 then return end -- Solo permite ejecutarlo desde la consola del server
    local seen = {}
    local collisions = 0
    local checksum_failures = 0
    
    print('^3[smoke] Iniciando generacion de 100 IBANs...^7')
    for i = 1, 100 do
    ...
    print(string.format('^2[smoke] 100 IBANs generated. Collisions=%d. ChecksumFailures=%d^7', collisions, checksum_failures))
end, true)
