-- =============================================================================
-- Admirals Bank — server/movements.lua
--
-- Helpers de bajo nivel sobre admirals_bank_movements (ledger inmutable
...
--     Reconciliación: SUM(amount) GROUP BY account vs admirals_bank_accounts.balance.
--     Delta != 0 indica inconsistencia. Usado por smoke atomicity verification.
--
-- Convention firmas:
--   - Queries usan placeholders '?' — Admirals.DB.* enforce prepared-only.
--   - amount es signed (positivo=ingreso, negativo=salida) per SSoT §4.2:553.
--
-- Referencias SSoT:
--   docs/technical/03_db_schema.md §4.2 (admirals_bank_movements DDL).
--   docs/technical/04_api_contracts.md §6 (DB layer).
-- =============================================================================

Admirals = Admirals or {}
Admirals.Bank = Admirals.Bank or {}
Admirals.Bank.Movements = Admirals.Bank.Movements or {}

local Movements = Admirals.Bank.Movements

-- Categorías canónicas ENUM admirals_bank_movements.category (SSoT §4.2:556 +
...
--   source_resource    : VARCHAR(64)? defaults 'admirals_bank'
...
function Movements.Insert(opts)
  ...
  local insert_id = Admirals.DB.Insert([[
    ...
    opts.source_resource or 'admirals_bank',
  })
...
-- vs admirals_bank_accounts.balance. Delta != 0 → ledger desbalanceado
...
function Movements.RecalcBalance(bank_account_id)
  ...
  local sum = Admirals.DB.Scalar([[
    SELECT COALESCE(SUM(amount), 0)
    FROM admirals_bank_movements
    WHERE bank_account_id = ?
  ]], { bank_account_id })

  local balance = Admirals.DB.Scalar([[
    SELECT balance FROM admirals_bank_accounts WHERE id = ?
  ]], { bank_account_id })
...
-- Boot announce.
-- =============================================================================
Admirals.Log.Info('Movements module ready (ENUM categories: %d valid)',
