-- =============================================================================
-- Admirals Bank — server/escrow.lua
--
...
--   Admirals.DB.Transaction([...queries]) con CHECK constraints como
...
--   1. INSERT admirals_bank_accounts (escrow técnico, type='escrow',
--      owner_* ambos NULL — habilitado por CHECK relax migration 006).
--   2. UPDATE admirals_bank_accounts.balance -= (amount + fee)   [buyer]
--        ↳ CHECK chk_...balance_nonneg garantiza INSUFFICIENT_FUNDS rollback.
--   3. UPDATE admirals_bank_accounts.balance += amount            [escrow account]
--   4. UPDATE admirals_bank_accounts.balance += fee               [system treasury]
--   5. INSERT admirals_bank_movements (buyer, -amount, 'escrow_lock')
--   6. INSERT admirals_bank_movements (buyer, -fee,    'escrow_fee')
--   7. INSERT admirals_bank_movements (escrow, +amount, 'escrow_lock')
--   8. INSERT admirals_bank_movements (system, +fee,    'escrow_fee')
--   9. INSERT admirals_escrows (status='locked' direct per S1.3 F3 decision).
--
-- Release TX (4 queries):
--   1. UPDATE admirals_bank_accounts.balance -= amount             [escrow account]
--   2. UPDATE admirals_bank_accounts.balance += amount             [recipient]
--   3. INSERT admirals_bank_movements (escrow, -amount, 'escrow_release'|'escrow_refund')
--   4. INSERT admirals_bank_movements (recipient, +amount, ídem)
--   5. UPDATE admirals_escrows SET status=<new>, released_to=<dir>,
--            released_by_account_id=<caller>, released_at=<ts>, updated_at=<ts>
--            WHERE id=? AND status='locked'  (guard: race contra 2nd release).
--
-- NOTA ENUM movements: admirals_bank_movements.category no tiene 'escrow_fee'
-- literal en SSoT §4.2:556 pero migration 003 lo aditiva de facto — verificado
-- en `@d:\theBigProject\resources\admirals_bank\server\movements.lua:43-49`
...
--   docs/technical/03_db_schema.md §4.3 (admirals_escrows — GAP SSoT
...
function Escrow.Create(buyer_cid, buyer_iban, seller_iban, amount, contract_id, release_condition, release_date, request_id)
  ...
  if not system_account_id then
    Admirals.Log.Error('Escrow.Create: system treasury account not found (iban=%s)',
      Config.EscrowFeeDestIban)
    Admirals.Metrics.Counter('bank.escrow.create.system_account_missing')
    return false, nil, 'SYSTEM_ACCOUNT_NOT_FOUND'
  end
  local system_acc = Admirals.DB.FetchOne([[
    SELECT id, balance FROM admirals_bank_accounts WHERE id = ?
  ]], { system_account_id })
  ...
  if not iban_ok then
    Admirals.Log.Error('Escrow.Create: IBAN.Generate failed: %s', tostring(iban_or_err))
    Admirals.Metrics.Counter('bank.escrow.create.iban_gen_failed')
  ...
  -- Q9: INSERT admirals_escrows con status='locked' direct per F3.
  ...
  if not tx_ok then
    Admirals.Log.Error('Escrow.Create TX crashed: buyer=%s seller=%s amount=%s fee=%s err=%s',
      buyer_iban, seller_iban, amount, fee, tostring(tx_result))
    Admirals.Metrics.Counter('bank.escrow.create.tx_crash')
    return false, nil, 'TX_CRASH'
  end
  if tx_result ~= true then
    ...
    if buyer_balance_pre >= total_debit then
      Admirals.Log.Warn('Escrow.Create RACE_DETECTED (CHECK violation or UNIQUE race): buyer=%s seller=%s amount=%s fee=%s escrow_id=%s',
        buyer_iban, seller_iban, amount, fee, escrow_id)
      Admirals.Metrics.Counter('bank.escrow.create.race_detected')
      return false, nil, 'RACE_DETECTED'
    end
    Admirals.Log.Error('Escrow.Create TX rollback: buyer=%s seller=%s amount=%s escrow_id=%s',
      buyer_iban, seller_iban, amount, escrow_id)
    Admirals.Metrics.Counter('bank.escrow.create.tx_rollback')
    return false, nil, 'TX_ROLLBACK'
  end

  Admirals.Metrics.Counter('bank.escrow.create.success')
  Admirals.Metrics.Observe('bank.escrow.create.tx_ms', tx_ms)
  ...
  -- ---------------------------------------------------------------------------
  -- 11. Publish event admirals:bank:escrow_created (schema §1.4 auto-decorated).
  -- ---------------------------------------------------------------------------
...
-- @param escrow_id   string — UUID v4 de admirals_escrows.id.
...
function Escrow.Release(caller_cid, escrow_id, direction, split_ratio, request_id)
  ...
  if not can_ok then
    Admirals.Log.Warn('Escrow.Release: FSM reject %s→%s for escrow=%s (reason=%s)',
      escrow.status, target_state, escrow_id, tostring(can_err))
    Admirals.Metrics.Counter('bank.escrow.release.invalid_state')
  ...
  if not auth_ok then
    Admirals.Metrics.Counter('bank.escrow.release.not_authorized')
  ...
  local recipient_row = Admirals.DB.FetchOne([[
    SELECT id, iban, balance FROM admirals_bank_accounts WHERE id = ?
  ]], { recipient_account_id })
  if not recipient_row then return false, nil, 'ESCROW_NOT_FOUND' end

  local escrow_bank_row = Admirals.DB.FetchOne([[
    SELECT id, iban, balance FROM admirals_bank_accounts WHERE id = ?
  ...
  if escrow_balance_pre < amount then
    ...
    Admirals.Log.Error('Escrow.Release: escrow balance %s < amount %s (escrow_id=%s) — data corruption',
      escrow_balance_pre, amount, escrow_id)
    Admirals.Metrics.Counter('bank.escrow.release.escrow_balance_insufficient')
    return false, nil, 'TX_ROLLBACK'
  end
  ...
  -- NOTA: admirals_bank_movements.request_nonce UQ? Revisando SSoT §4.2 —
  ...
  if direction == 'seller' then
    ...
    -- via admirals_escrows.released_to + audit category. S2+ ALTER ENUM ADD 'escrow_refund'.
    movement_category = 'escrow_release'
  end
  ...
  -- Q5: UPDATE admirals_escrows status + audit fields.
  ...
  if not tx_ok then
    Admirals.Log.Error('Escrow.Release TX crashed: escrow=%s dir=%s err=%s',
      escrow_id, direction, tostring(tx_result))
    Admirals.Metrics.Counter('bank.escrow.release.tx_crash')
    return false, nil, 'TX_CRASH'
  end
  if tx_result ~= true then
    Admirals.Log.Error('Escrow.Release TX rollback: escrow=%s dir=%s',
      escrow_id, direction)
    Admirals.Metrics.Counter('bank.escrow.release.tx_rollback')
    return false, nil, 'TX_ROLLBACK'
  end
  ...
  if not escrow_post or escrow_post.status ~= target_state then
    Admirals.Log.Warn('Escrow.Release post-UPDATE race: escrow=%s expected=%s actual=%s',
      escrow_id, target_state, escrow_post and escrow_post.status or 'nil')
    Admirals.Metrics.Counter('bank.escrow.release.race_lost')
    return false, nil, 'INVALID_STATE'
  end

  Admirals.Metrics.Counter(('bank.escrow.release.success.%s'):format(direction))
  Admirals.Metrics.Observe('bank.escrow.release.tx_ms', tx_ms)
...
-- Boot announce.
-- =============================================================================
Admirals.Log.Info('Escrow module ready (fee_rate=%.2f%%, fee_range=[%.2f, %.2f]€, expiry_default=%ds)',
