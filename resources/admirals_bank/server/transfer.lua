-- =============================================================================
-- Admirals Bank — server/transfer.lua
--
-- Lógica core de transferencias player→player (callback C002 backend).
--
-- Atomicidad (S1.2 fix-and-validate — Opción D pura):
--   `Admirals.DB.Transaction` envuelve 4 queries en una MySQL TX:
--     1. UPDATE admirals_bank_accounts (debit from)
--     2. UPDATE admirals_bank_accounts (credit to)
--     3. INSERT admirals_bank_movements (row debe — amount negativo)
--     4. INSERT admirals_bank_movements (row haber — amount positivo)
...
--
-- Atomicity guarantee:
--   CHECK constraint chk_admirals_bank_accounts_balance_nonneg (migration 005)
...
--   docs/technical/03_db_schema.md §4.2 (admirals_bank_movements signed amount).
...
Admirals.Bank.Transfer = Admirals.Bank.Transfer or {}
...
--   TX_CRASH            — pcall trapped error en Admirals.DB.Transaction
...
function Transfer.Execute(from_cid, from_iban, to_iban, amount, concept, request_id)
  ...
  -- Q1: Debit from. CHECK constraint chk_admirals_bank_accounts_balance_nonneg
  ...
  local tx_ok, tx_result = pcall(function()
    return Admirals.DB.Transaction({ q_debit, q_credit, q_mov_debit, q_mov_credit })
  end)
  local tx_ms = GetGameTimer() - start_ms

  if not tx_ok then
    Admirals.Log.Error('Transfer.Execute TX crashed: from=%s to=%s amount=%s err=%s',
      from_iban, to_iban, amount, tostring(tx_result))
    Admirals.Metrics.Counter('bank.transfer.tx_crash')
  ...
  if tx_result ~= true then
    ...
    if from_balance_pre >= amount then
      Admirals.Log.Warn('Transfer.Execute RACE_DETECTED (CHECK violation): from=%s to=%s amount=%s pre_balance=%s txid=%s',
        from_iban, to_iban, amount, from_balance_pre, transaction_id)
      Admirals.Metrics.Counter('bank.transfer.race_detected')
      return false, nil, 'RACE_DETECTED'
    end
    Admirals.Log.Error('Transfer.Execute TX rollback (other): from=%s to=%s amount=%s txid=%s',
      from_iban, to_iban, amount, transaction_id)
    Admirals.Metrics.Counter('bank.transfer.tx_rollback')
    return false, nil, 'TX_ROLLBACK'
  end

  Admirals.Metrics.Counter('bank.transfer.success')
  Admirals.Metrics.Observe('bank.transfer.tx_ms', tx_ms)
  ...
  -- ---------------------------------------------------------------------------
  -- 10. Publish event admirals:bank:transfer_completed.
...
-- Boot announce.
-- =============================================================================
Admirals.Log.Info('Transfer module ready (fee=0€, max=%d€, rate=bank.write)', _MAX_AMOUNT)
