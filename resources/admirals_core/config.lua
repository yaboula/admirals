...
Admirals.Config = Admirals.Config or {}
...
-- -----------------------------------------------------------------------------
Config.Env = GetConvar('admirals_env', 'development')
...
Config.LogLevel = GetConvar('admirals_core_log_level', _default_log)

-- Ring buffer size — admirals_log_dump devuelve las últimas N entries.
...
--   always   — persistir TODO evento a admirals_event_log (muy caro, dev only).
...
Config.BusAuditMode = GetConvar('admirals_core_bus_audit', 'config')
...
-- Subscription wildcard — permitir Subscribe('admirals:*') / ('admirals:bank:*').
...
-- Principal ACE requerido para los comandos admirals_* admin-only.
-- Registered en server.cfg:
--   add_ace group.admin command.admirals_log_dump allow
--   add_ace group.admin command.admirals_log_level allow
--   add_ace group.admin command.admirals_metrics allow
--   ...
Config.AdminAcePrefix = 'command.admirals_'

-- Commands registrados por logger / metrics / init.
Config.AdminCommands = {
  'admirals_log_dump',      -- últimas N entries ring buffer.
  'admirals_log_level',     -- cambia LogLevel runtime.
  'admirals_log_clear',     -- limpia ring buffer.
  'admirals_metrics',       -- dump counters + histograms.
  'admirals_metrics_reset', -- reset all metrics a 0.
  'admirals_core_status',   -- muestra ready state + version + migrations.
}
