-- =============================================================================
-- Admirals Core — server/migrations.lua
--
...
--
-- API pública:
--   Admirals.Migrations.RunAll()              → { applied, skipped, errors }
--   Admirals.Migrations.ListApplied()         → { { version, filename, applied_at, checksum } }
--   Admirals.Migrations.IsApplied(version)    → boolean
--
-- Protocolo (per docs/technical/03_db_schema.md §16):
--   1. Bootstrap: ensure `admirals_schema_versions` table exists (first-run).
...
--      b. SELECT admirals_schema_versions WHERE version = N.
--      c. Si aplicada:
...
--         - Si exitoso → INSERT en admirals_schema_versions.
...
--   dentro de strings). Los .sql files Admirals siguen esta convención.
--
-- SHA-256:
...
-- -----------------------------------------------------------------------------
-- Internal — bootstrap admirals_schema_versions table (first-run special case).
--
...
local function _ensure_schema_versions_table()
  ...
  local exists = DB.Scalar([[
    ...
    WHERE table_schema = DATABASE() AND table_name = 'admirals_schema_versions'
  ]], {})
...
function M.RunAll()
  ...
  Log.Debug('admirals_schema_versions exists pre-run: %s', tostring(had_table))
  ...
  for _, filename in ipairs(Config.MigrationsFiles) do
    ...
    if not _ensure_schema_versions_table() then
      local msg = ('[Migrations] %s applied but admirals_schema_versions still missing'):format(filename)
      Log.Error(msg)
    ...
    local ok_track, track_err = pcall(function()
      DB.Insert([[
        INSERT INTO admirals_schema_versions
