-- =============================================================================
-- Admirals Core — server/metrics.lua
--
-- Instrumentación counters + histograms in-memory para admirals_core.
-- Exporta a /admirals_metrics admin command. Sirve como base para
-- futura export Prometheus (Oleada 2+).
--
-- API pública:
--   Admirals.Metrics.Counter(key, delta?)              — incrementa counter.
--   Admirals.Metrics.Observe(key, value_ms)            — histogram sample.
--   Admirals.Metrics.Gauge(key, value)                 — set gauge absoluto.
--   Admirals.Metrics.Get(key)                          — inspecciona entry.
--   Admirals.Metrics.Snapshot()                        — full dump table.
--   Admirals.Metrics.Reset()                           — zero all.
--
-- Histogram:
...
--
-- Admin command:
--   /admirals_metrics         — imprime snapshot completo.
--   /admirals_metrics_reset   — zero todos counters + histograms.
--
-- Referencias SSoT:
...
-- /admirals_metrics — dump completo.
RegisterCommand('admirals_metrics', function(source)
  if not _is_admin(source) then return end

  local snap = Metrics.Snapshot()
  print('^5=== Admirals Core — Metrics Snapshot ===^7')
...
-- /admirals_metrics_reset.
RegisterCommand('admirals_metrics_reset', function(source)
  if not _is_admin(source) then return end
  Metrics.Reset()
  print('^2[Admirals] Metrics reset.^7')
end, true)
