-- =============================================================================
-- Admirals Core — server/logger.lua
--
-- Logger estructurado para admirals_core + resto de resources admirals_*.
--
-- API pública:
--   Admirals.Log.Debug(fmt, ...)
--   Admirals.Log.Info(fmt, ...)
--   Admirals.Log.Warn(fmt, ...)
--   Admirals.Log.Error(fmt, ...)
--   Admirals.Log.Audit(entry_table)       — escribe a ring buffer siempre +
--                                           console si level permite.
--   Admirals.Log.GetRingBuffer([n])       — oldest-first; opcional últimas n.
--   Admirals.Log.Clear()                  — vacía ring buffer.
--   Admirals.Log.SetLevel(level_string)   — runtime override.
--
-- Admin commands (ACE-gated):
--   /admirals_log_dump [n]   — imprime últimas n entries (default 50).
--   /admirals_log_level <lv> — cambia LogLevel runtime.
--   /admirals_log_clear      — vacía ring buffer.
--
-- Formato console:
...
-- -----------------------------------------------------------------------------
-- Resource name detection — útil cuando admirals_bank / admirals_tablet
-- requieran el logger más adelante (S1+). Por ahora "admirals_core".
-- -----------------------------------------------------------------------------
local _resource_name = GetCurrentResourceName() or 'admirals_core'
...
-- -----------------------------------------------------------------------------
-- Public — Clear — vacía ring buffer (útil para tests + /admirals_log_clear).
-- -----------------------------------------------------------------------------
function Log.Clear()
  _ring = {}
  _ring_head = 1
...
-- -----------------------------------------------------------------------------
-- Public — Size — útil para tests + /admirals_metrics.
-- -----------------------------------------------------------------------------
function Log.Size()
  return _ring_size
end
...
-- /admirals_log_dump [n] — imprime últimas n entries.
RegisterCommand('admirals_log_dump', function(source, args)
  if not _is_admin(source) then
    if source ~= 0 then
      TriggerClientEvent('chat:addMessage', source,
        { args = { '^1[Admirals]', 'Access denied.' } })
    end
    return
  end
  ...
  print(string.format('^5=== Admirals Core — Log Dump (last %d of %d) ===^7',
    #entries, _ring_size))
...
-- /admirals_log_level <debug|info|warn|error>
RegisterCommand('admirals_log_level', function(source, args)
  if not _is_admin(source) then return end
  local level = args[1]
  if not level then
    print(string.format('Usage: admirals_log_level <debug|info|warn|error>. Current: %s',
      _current_level))
    return
  end
  Log.SetLevel(level)
...
-- /admirals_log_clear — vacía ring buffer.
RegisterCommand('admirals_log_clear', function(source)
  if not _is_admin(source) then return end
  Log.Clear()
  print('^2[Admirals] Log ring buffer cleared.^7')
end, true)
