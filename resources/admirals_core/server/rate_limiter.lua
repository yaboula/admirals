-- =============================================================================
-- Admirals Core — server/rate_limiter.lua
--
...
--
-- API pública:
--   Admirals.Rate.Check(identity, bucket_key) → true (allowed) | false (blocked)
--   Admirals.Rate.RegisterBucket(key, { max, window_sec })
--   Admirals.Rate.GetBucket(key)                → def | nil
--   Admirals.Rate.Reset(identity?, bucket?)     — reset all, identity, o par.
--   Admirals.Rate.Stats()                       → { tracked_identities, ... }
--
-- Identity:
...
function Rate.RegisterBucket(key, def)
  ...
  if def.max < 1 or def.window_sec < 1 then
    error('[Admirals.Rate.RegisterBucket] max and window_sec must be >= 1', 2)
  end
