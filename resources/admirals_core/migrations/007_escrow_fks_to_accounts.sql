...
-- Description:
--   Corrige FK targets de `admirals_escrows.buyer_account_id` y
--   `seller_account_id` — apuntaban incorrectamente a
--   `admirals_bank_accounts(id)` en migration 006 (error conceptual).
--
--   Semántica correcta (alineada con el código S1.3 ya escrito):
--     - buyer_account_id, seller_account_id → admirals_accounts(id)
...
--     - escrow_account_id → admirals_bank_accounts(id)
--       (cuenta técnica server-managed — FK correcto desde 006, no se toca).
--
-- Dependencies:
--   - 006_escrow_schema.sql (crea FKs originales a admirals_bank_accounts).
--
-- Reversible:
--   Sí en dev. DROP nuevos FKs + re-ADD originales a admirals_bank_accounts(id).
--
-- Symptom pre-007:
--   INSERT INTO admirals_escrows ... VALUES (..., 'a816aa90-...', ...) →
--   "Cannot add or update a child row: a foreign key constraint fails
--   (admirals_escrows, CONSTRAINT fk_admirals_escrows_buyer FOREIGN KEY
--   (buyer_account_id) REFERENCES admirals_bank_accounts (id))"
--
-- DECISIONES TÉCNICAS:
--
...
--         admirals_accounts. Red de operaciones atomic DDL (InnoDB).
--       - Existing DB S1.3 pre-fix: 006 aplicado, 0 rows en admirals_escrows
...
--
--   D4. Índices `idx_admirals_escrows_buyer`, `idx_admirals_escrows_seller`
...
--       pattern que admirals_bank_accounts.owner_account_id/owner_company_id).
--       Out of scope S1.3.
-- ============================================================================
...
-- ----------------------------------------------------------------------------
-- 2. Add FKs correctos apuntando a admirals_accounts(id).
--
...
ALTER TABLE admirals_escrows
  ADD CONSTRAINT fk_admirals_escrows_buyer
    FOREIGN KEY (buyer_account_id) REFERENCES admirals_accounts(id)
    ON DELETE RESTRICT ON UPDATE CASCADE;

ALTER TABLE admirals_escrows
  ADD CONSTRAINT fk_admirals_escrows_seller
    FOREIGN KEY (seller_account_id) REFERENCES admirals_accounts(id)
    ON DELETE RESTRICT ON UPDATE CASCADE;
