...
--     1. admirals_bank_accounts   (cuentas IBAN personales/empresariales)
--     2. admirals_bank_movements  (ledger inmutable, PARTITIONED mensual)
--
--   `admirals_escrows` se difiere a S1.3 (ADR — green-light founder May 2 2026).
...
--   - 002_foundation_tables (admirals_accounts → FK target).
--
...
--   docs/technical/03_db_schema.md §4.2 (admirals_bank_movements PARTITIONED).
...
--
--   D3. FK `admirals_bank_accounts.owner_company_id → admirals_companies(id)`
--       DEFERRED. La tabla `admirals_companies` no existe aún (se creará S2+).
--       Cuando S2 cree `admirals_companies`, su migration añadirá el FK via
...
--       `admirals_escrows` se difiere — esto permite a S1.3 simplemente
--       INSERT un account con type='escrow' sin necesitar nuevo ENUM ALTER.
-- ============================================================================


-- ----------------------------------------------------------------------------
-- 1. admirals_bank_accounts
--
...
CREATE TABLE IF NOT EXISTS admirals_bank_accounts (
  ...
  owner_company_id      CHAR(36)        NULL     COMMENT 'NULL si type=personal — FK a admirals_companies se añade en S2+',
  ...
  KEY idx_admirals_bank_accounts_type_active (type, closed_at),
  ...
  -- FK admirals_companies se añade S2+ via ALTER TABLE (entrega aditiva):
  --   ALTER TABLE admirals_bank_accounts
  --     ADD CONSTRAINT fk_admirals_bank_accounts_owner_company
  --     FOREIGN KEY (owner_company_id) REFERENCES admirals_companies(id)
  --     ON DELETE RESTRICT ON UPDATE CASCADE;

  CONSTRAINT chk_admirals_bank_accounts_owner_xor CHECK (
...
-- ----------------------------------------------------------------------------
-- 2. admirals_bank_movements (PARTITIONED RANGE BY occurred_at)
--
...
CREATE TABLE IF NOT EXISTS admirals_bank_movements (
  ...
  source_resource       VARCHAR(64)     NOT NULL COMMENT 'admirals_bank, admirals_market, ...',
