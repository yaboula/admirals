...
-- Description:
--   (a) Relaxa CHECK constraint de admirals_bank_accounts ownership para
...
--       Nombre anterior: `chk_admirals_bank_accounts_owner_xor` (migration 003).
--       Nombre nuevo:    `chk_admirals_bank_accounts_owner_xor_or_escrow`.
--
--   (b) Crea tabla `admirals_escrows` — holdings transaccionales entre buyer
--       y seller con fee retenido. Referenced por FSM escrow_lifecycle per
--       `docs/technical/05_state_machines.md` §4.1.
--
-- Dependencies:
--   - 003_bank_schema.sql (admirals_bank_accounts existe con constraint original).
--   - 005_balance_nonneg_check.sql (chk balance >= 0 preservado, no-op aquí).
--
-- Reversible: sí en dev (DROP TABLE admirals_escrows + restore constraint
...
--
--   D1. CONSTRAINT chk_admirals_bank_accounts_owner_xor_or_escrow —
...
--
--   D2. `admirals_escrows.status` ENUM 5 valores per SSoT §05 §4.1:
...
--       `exports.admirals_bridges:IsIdemReplay` (consistency con S1.2 C002).
--
...
--        check en `admirals_core/server/migrations.lua:253` enforce tampering
--        detection.
-- ============================================================================


-- ----------------------------------------------------------------------------
-- 1. Relax CHECK constraint admirals_bank_accounts — add `type='escrow'` branch.
--
-- Pre-006: `chk_admirals_bank_accounts_owner_xor` (migration 003 línea 107)
...
--
-- Post-006: `chk_admirals_bank_accounts_owner_xor_or_escrow` 3-ramas:
...
ALTER TABLE admirals_bank_accounts
  DROP CONSTRAINT IF EXISTS chk_admirals_bank_accounts_owner_xor;

ALTER TABLE admirals_bank_accounts
  DROP CONSTRAINT IF EXISTS chk_admirals_bank_accounts_owner_xor_or_escrow;

ALTER TABLE admirals_bank_accounts
  ADD CONSTRAINT chk_admirals_bank_accounts_owner_xor_or_escrow CHECK (
    type = 'escrow'
...
-- ----------------------------------------------------------------------------
-- 2. admirals_escrows — holdings transaccionales buyer↔seller con fee retained.
--
...
--
-- Cada row vincula:
--   - buyer_account_id    → admirals_bank_accounts.id (del comprador)
--   - seller_account_id   → admirals_bank_accounts.id (del vendedor)
--   - escrow_account_id   → admirals_bank_accounts.id (cuenta técnica server-
--                           managed, type='escrow', creada en la misma TX
--                           atomic que INSERT admirals_escrows).
--
...
-- ----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS admirals_escrows (
  id                        CHAR(36)        NOT NULL COMMENT 'UUID v4 application-generated',
  status                    ENUM('created','locked','released','refunded','disputed') NOT NULL,

  buyer_account_id          CHAR(36)        NOT NULL COMMENT 'FK admirals_bank_accounts.id (buyer)',
  seller_account_id         CHAR(36)        NOT NULL COMMENT 'FK admirals_bank_accounts.id (seller)',
  escrow_account_id         CHAR(36)        NOT NULL COMMENT 'FK admirals_bank_accounts.id (escrow técnico server-managed)',
  ...
  contract_id               VARCHAR(64)     NULL     COMMENT 'Ref contrato B2B (admirals_contracts S2+, NULL S1.3)',
  ...
  PRIMARY KEY (id),
  UNIQUE KEY uq_admirals_escrows_request_nonce (request_nonce),

  KEY idx_admirals_escrows_buyer (buyer_account_id),
  KEY idx_admirals_escrows_seller (seller_account_id),
  KEY idx_admirals_escrows_escrow_account (escrow_account_id),
  KEY idx_admirals_escrows_status_expires (status, expires_at),
  KEY idx_admirals_escrows_contract (contract_id),

  CONSTRAINT fk_admirals_escrows_buyer
    FOREIGN KEY (buyer_account_id) REFERENCES admirals_bank_accounts(id)
    ON DELETE RESTRICT ON UPDATE CASCADE,
  CONSTRAINT fk_admirals_escrows_seller
    FOREIGN KEY (seller_account_id) REFERENCES admirals_bank_accounts(id)
    ON DELETE RESTRICT ON UPDATE CASCADE,
  CONSTRAINT fk_admirals_escrows_escrow_account
    FOREIGN KEY (escrow_account_id) REFERENCES admirals_bank_accounts(id)
    ON DELETE RESTRICT ON UPDATE CASCADE,

  CONSTRAINT chk_admirals_escrows_amount_positive CHECK (amount > 0),
  CONSTRAINT chk_admirals_escrows_fee_nonneg CHECK (fee_charged >= 0)
