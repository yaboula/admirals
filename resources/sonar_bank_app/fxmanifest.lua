...
server_scripts {
  ...
  'server/state/statebags.lua',         -- 22. M004 §2.2.2 playerJoining lazy publish hook

  -- 5. Services (business logic + FSM orchestration — depends repos + lib).
  'server/services/bootstrap_service.lua',   -- 23. REQ-FE-001 (must load BEFORE other services that invalidate it)
  'server/services/recipients_service.lua',  -- 24. REQ-FE-002
  'server/services/transfer_service.lua',    -- 25.
  'server/services/account_service.lua',     -- 26.
  'server/services/loan_service.lua',        -- 27.
  'server/services/recurring_service.lua',   -- 28.
  'server/services/portfolio_service.lua',   -- 29.
  'server/services/card_service.lua',        -- 30.
  'server/services/admin_service.lua',       -- 31.

  -- 6. Events (NetEvent emitters + audit emit helpers — depends lib + Enums).
  'server/events/netevents.lua',        -- 32.
  'server/events/audit_emit.lua',       -- 33.

  -- 7. NUI bridge (server stub for client config snapshot).
  'server/nui/bridge.lua',              -- 34.

  -- 8. Callbacks (Step E — 49 endpoints).
  --    _wrap.lua MUST load first (all callback files depend on Wrap.Register).
  'server/callbacks/_wrap.lua',         -- 35.
  'server/callbacks/bootstrap.lua',     -- 36. C001, C001b, NUI_CONFIG       (3)
  'server/callbacks/account.lua',       -- 37. C002, C003, C015, C016, C019,
                                        --      C020, C021, C037, C038, C039 (10)
  'server/callbacks/transfer.lua',      -- 38. C005, C006, C007, C008        (4)
  'server/callbacks/recipients.lua',    -- 39. C009, C010, C011, C012        (4)
  'server/callbacks/loan.lua',          -- 40. C022, C023, C024, C025, C026  (5)
  'server/callbacks/recurring.lua',     -- 41. C013, C014, C017, C018a/b     (5)
  'server/callbacks/portfolio.lua',     -- 42. C027, C028, C029              (3)
  'server/callbacks/card.lua',          -- 43. C030, C032, C033, C034, C040  (5)
  'server/callbacks/admin.lua',         -- 44. C035, C036, C036b, C031, C041,
                                        --      C042, C043, C044, C045, C046  (10)

  -- 9. Boot orchestration (Step F).
  --    smoke + cron must load BEFORE init (init references them in phase 1/2).
  'server/boot/smoke.lua',              -- 45.
