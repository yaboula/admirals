...
import { maskCidDisplay, maskIbanPanel, maskMoneyDisplay, maskSignedMoneyDisplay } from '@/lib/privacy'
...
interface Props {
  detail: GovtCitizenDetail
  isFetching?: boolean
}

const STATUS_TONE: Record<GovtCitizenStatus, { tone: 'success' | 'warning' | 'danger' | 'neutral'; key: TranslationKey }> = {
  active: { tone: 'success', key: 'govt.census.status.active' },
  flagged: { tone: 'warning', key: 'govt.census.status.flagged' },
  sanctioned: { tone: 'danger', key: 'govt.census.status.sanctioned' },
  exempt: { tone: 'neutral', key: 'govt.census.status.exempt' },
}
...
const ACTIVITY_ICON: Record<GovtActivityType, typeof ArrowUpRight> = {
  ...
  tax_payment: Receipt,
  flag_raised: Flag,
  sanction_applied: ShieldOff,
  subsidy_received: HandCoins,
}

const ACTIVITY_LABEL: Record<GovtActivityType, TranslationKey> = {
  transfer_out: 'govt.census.activity.transfer_out',
  transfer_in: 'govt.census.activity.transfer_in',
  card_charge: 'govt.census.activity.card_charge',
  tax_payment: 'govt.census.activity.tax_payment',
  flag_raised: 'govt.census.activity.flag_raised',
  sanction_applied: 'govt.census.activity.sanction_applied',
  subsidy_received: 'govt.census.activity.subsidy_received',
}
...
export function CitizenDetail({ detail, isFetching }: Props) {
  ...
  const cidDisplay = streamerMode ? maskCidDisplay(detail.cid) : detail.cid
  const ibanDisplay = streamerMode ? maskIbanPanel(detail.primaryIban) : detail.primaryIban
  const holdingsDisplay = streamerMode ? maskMoneyDisplay() : money(detail.totalHoldings)
  ...
  return (
    ...
    >
      <GovtCard variant="hero" padding="lg">
        <div className="flex items-center gap-4">
          ...
          <div className="min-w-0 flex-1">
            <h2 className="truncate text-2xl font-light leading-tight tracking-[-0.04em] text-[var(--color-govt-text-primary)]">{detail.alias}</h2>
        ...
        <div className="mt-3 flex flex-wrap items-center gap-2">
          ...
          <GovtPill tone="seal" size="md">{`${t('govt.census.detail.residency')}: ${number(detail.residencyDays)}d`}</GovtPill>
        </div>
      </GovtCard>
      ...
      <div className="grid gap-3 lg:grid-cols-[minmax(0,1.1fr)_minmax(0,0.9fr)]">
        ...
        <GovtCard variant="glass" padding="md">
          <SectionHeader icon={AlertTriangle} label={t('govt.census.detail.riskTitle')} />
      ...
      <GovtCard variant="outline" padding="md" className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-xs text-[var(--color-govt-text-secondary)]">{t('govt.census.detail.actionsHint')}</p>
        </div>
        <div className="flex flex-wrap gap-2">
          <ActionButton icon={Gavel} label={t('govt.census.detail.openSanctions')} />
          <ActionButton icon={Banknote} label={t('govt.census.detail.openSubsidy')} />
        </div>
      </GovtCard>
    </motion.div>
  )
...
function SectionHeader({ icon: Icon, label }: { icon: typeof Wallet; label: string }) {
  return (
...
function ActionButton({ icon: Icon, label }: { icon: typeof Gavel; label: string }) {
  const { t } = useI18n()
  return (
    <button
      type="button"
