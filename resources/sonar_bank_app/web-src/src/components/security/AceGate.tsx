...
export function AceLockedState({
  ...
  description = 'Esta superficie está protegida por permisos ACE y no está disponible para tu sesión actual.',
  className,
}: AceLockedStateProps) {
  return (
    <Card variant="glass" padding="lg" className={cn('border-white/10 text-center', className)}>
      <CardContent className="items-center gap-3">
        ...
        <div className="space-y-1">
          <CardEyebrow>ACE gate</CardEyebrow>
          <CardTitle className="text-base">{title}</CardTitle>
          <p className="mx-auto max-w-[36ch] text-sm leading-relaxed text-text-tertiary">{description}</p>
        </div>
      </CardContent>
    </Card>
  )
