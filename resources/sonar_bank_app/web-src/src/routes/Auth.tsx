...
const FEATURE_CARDS: Array<{ icon: LucideIcon; title: TranslationKey; description: TranslationKey; tone: string }> = [
  { icon: TrendingUp, title: 'auth.investments', description: 'auth.investmentsDescription', tone: 'rgb(41, 194, 113)' },
  { icon: Bitcoin, title: 'auth.crypto', description: 'auth.cryptoDescription', tone: 'rgb(95, 161, 243)' },
  { icon: BriefcaseBusiness, title: 'auth.business', description: 'auth.businessDescription', tone: 'rgb(223, 166, 53)' },
]
          ...
          {bars.map((height, index) => {
            ...
            return (
              <span key={index} className="flex h-full items-end justify-center">
                <span
                  className={cn('block w-full max-w-[17px] rounded-t-full rounded-b-md transition-colors', active ? 'shadow-[0_12px_24px_-14px_rgb(41, 194, 113)]' : '')}
                  style={{
                    height: `${height}%`,
                    background: active
